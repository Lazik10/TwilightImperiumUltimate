...
/// <summary>
/// Win a game with each PoK faction.
/// </summary>
[AchievementEvaluator(AchievementName.TheProphesiedKing)]
public sealed class TheProphesiedKingAchievementEvaluator(
    IAchievementRepository achievementRepository,
    ITiglUserRepository tiglUserRepository)
    : IAchievementEvaluator
{
    private const int ProphecyOfKingsFactionsCount = 7;

    private static readonly HashSet<TiglFactionName> ProphecyOfKingsFactions = Enum.GetValues<TiglFactionName>()
        .Where(f => f >= TiglFactionName.TheArgentFlight && f <= TiglFactionName.TheVuilRaithCabal)
        .ToHashSet();
    ...
    public async Task EvaluateAsync(MatchReport matchReport, AchievementName achievementName, IReadOnlyCollection<TiglUserAchievement> usersAchievements, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var winner in winners)
        {
            ...
            var uniqueFactionWins = games.SelectMany(g => g.PlayerResults)
                .Where(pr => pr.TiglUserId == winner.TiglUserId && ProphecyOfKingsFactions.Contains(pr.Faction))
                .Select(x => x.Faction)
                .Distinct()
                .ToHashSet();
            ...
            if (uniqueFactionWins.Count == ProphecyOfKingsFactionsCount && uniqueFactionWins.SetEquals(ProphecyOfKingsFactions))
