...
/// </summary>
[AchievementEvaluator(AchievementName.GottaCatchEmAllPartTwo)]
public sealed class GottaCatchEmAllPartTwoAchievementEvaluator(
    IAchievementRepository achievementRepository,
    ITiglUserRepository tiglUserRepository)
    : IAchievementEvaluator
{
    private const int StandardFactionsCount = 79; // No Keleres

    private static readonly HashSet<TiglFactionName> StandardFactions = Enum.GetValues<TiglFactionName>()
        .Where(f => f >= TiglFactionName.TheArborec && f <= TiglFactionName.Homebrew)
        .ToHashSet();
