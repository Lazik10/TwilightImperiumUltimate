...
/// <summary>
/// Win a game with each TI-1 faction (Hacan, Sol, Xxcha, Barony, Jol-Nar, Sardakk).
/// </summary>
[AchievementEvaluator(AchievementName.IAmAnOG)]
public sealed class IamAnOGAchievementEvaluator(
    IAchievementRepository achievementRepository,
    ITiglUserRepository tiglUserRepository)
    : IAchievementEvaluator
{
    private const int OgFactionCount = 6;

    private static readonly HashSet<TiglFactionName> OgFactions =
    [
    ...
    public async Task EvaluateAsync(MatchReport matchReport, AchievementName achievementName, IReadOnlyCollection<TiglUserAchievement> usersAchievements, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var winner in winners)
        {
            ...
            var uniqueFactionWins = games.SelectMany(g => g.PlayerResults)
                .Where(pr => pr.TiglUserId == winner.TiglUserId && OgFactions.Contains(pr.Faction))
                .Select(x => x.Faction)
                .Distinct()
                .ToHashSet();
            ...
            if (uniqueFactionWins.Count == OgFactionCount && uniqueFactionWins.SetEquals(OgFactions))
