...
{
    private static readonly SemaphoreSlim LeaderboardRefreshLock = new(1, 1);

    public async Task CreateLeaderboard(int seasonEnded, CancellationToken cancellationToken)
    {
        await LeaderboardRefreshLock.WaitAsync(cancellationToken);
        ...
        try
        {
            ...
            await db.SeasonLeaderboard
                .Where(x => x.Season == seasonEnded)
        ...
        {
            LeaderboardRefreshLock.Release();
        }
    }
}
